...
if(!empty($data) && !empty($godzina))//sprawdzamy czy pole data nie jest puste
{
    ...
    echo "<div class=\"flex-container\">";
                    ...
                    for($a=0;$a<$liczenie;$a++)
        {
            ...
            $spr_wykorzystanych = mysql_query("select czyje_id, kiedy from (SELECT czyje_id, wykorzystane_nadgodziny.kiedy FROM wykorzystane_nadgodziny left join nadgodziny using (idNadgodziny)
                                        union all
                                        SELECT kto_mial as czyje_id, wykorzystane_sluzby.kiedy FROM wykorzystane_sluzby left join sluzby using (idSluzby) ) as sprawdzenie
                ...
                <table id="tabela">
                    ...
                    <tbody>
                        <tr class="blekitne">
                            ...
                            <td><input type="text" name="godzina[]" class="ggodzin" placeholder="2.5" pattern="((\d{1,2}\.[5])|(\d{1,2}))" required="true" size="19" id="godzina-1" min="1" max="8" step="0.5"></td>
                        </tr>
                    </tbody>
                </table>
...
<?php
wykorzystaj_nadgodziny($_GET['profil'], $_POST['data'], $_POST['godzina']);
    //dodajNadgodziny();
...
$(document).ready(function() {
    ...
    $('#dodajWiersz').click(function() {
        ...
        //druga kom�rka
        var f2  = '<td><input type="text" name="godzina[] '+liczba+'" class="ggodzin" placeholder="2.5" required="true" size="19" id="godzina-'+liczba+'" min="1" max="8" step="0.5"></td>';
